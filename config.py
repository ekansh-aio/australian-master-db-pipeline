...
ROLE_CLASSIFICATION_CONFIG = {
    ...
    # Model configuration
    "model_name": os.getenv("ROLE_MODEL_NAME", "distilbert-base-uncased"),  # Base model or path to fine-tuned
    ...
    # Role definitions (customize these for your use case)
    "role_definitions": [
        role.strip() 
        for role in os.getenv(
            "ROLE_DEFINITIONS",
            "decision,facts,analysis,procedural_history,issue,reasoning,holding,disposition,dissent,other"
        ).split(',')
    ],
    ...
    "confidence_threshold": float(os.getenv("ROLE_CONFIDENCE_THRESHOLD", "0.0")),  # 0.0 = no threshold
}
